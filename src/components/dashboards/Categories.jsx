...
export default function Categories() {
    ...
    async function fetchCategoriesAndRatings() {
        setIsLoading(true);
        try {
            const categoryResponse = await axios.get(DashboardAPIs.getDashboardCategoriesByDashboardId(dashboardId), {
    ...
    async function fetchRatings(categories) {
        const ratingsResponse = await Promise.all(categories.map(async category => {
            const userRating = await axios.get(`${DashboardAPIs.getDashboardCategoryByCategoryId(dashboardId, category.id)}/priorities/user`, {
                headers: { Authorization: `Bearer ${jwtToken}` },
            }).catch(() => ({ data: { priority: 0 } }));

            const calculatedRating = await axios.get(`${DashboardAPIs.getDashboardCategoryByCategoryId(dashboardId, category.id)}/priorities/calculate`, {
                headers: { Authorization: `Bearer ${jwtToken}` },
            }).catch(() => ({ data: 0 }));
        ...
        const newRatings = ratingsResponse.reduce((acc, rating) => {
            acc[rating.id] = { myRating: rating.userRating, calculatedRating: rating.calculatedRating };
            return acc;
        }, {});
    ...
    const fetchCalculatedRatings = async (categories) => {
        try {
            const ratingsPromises = categories.map(category =>
                axios.get(`${DashboardAPIs.getDashboardCategoryByCategoryId(dashboardId, category.id)}/priorities/calculate`, {
                    headers: { Authorization: `Bearer ${jwtToken}` },
            ...
            const newCalculatedRatings = ratingsResponses.map((response, index) => ({
                [categories[index].id]: { ...ratings[categories[index].id], calculatedRating: response.data }
            }));
    ...
    const fetchUserRatings = async (categories) => {
        try {
            const userRatingsPromises = categories.map(category =>
                axios.get(`${DashboardAPIs.getDashboardCategoryByCategoryId(dashboardId, category.id)}/priorities/user`, {
                    headers: { Authorization: `Bearer ${jwtToken}` },
            ...
            const newUserRatings = userRatingsResponses.map((response, index) => ({
                [categories[index].id]: { ...ratings[categories[index].id], myRating: response.data.priority }
            }));
            setRatings(prevRatings => ({
                ...prevRatings,
    ...
    const handleRateCategory = async (categoryId, priorityValue) => {
        ...
        try {
            await axios.post(url, {}, { headers: { Authorization: `Bearer ${jwtToken}` } });
            setRatings(prev => ({
                ...prev,
                            ...
                            {categories.map((category) => (
                                <ListItem key={category.id}>
                                    ...
                                    <FormControl style={{ margin: '0 20px', width: 100 }}>
                                        <InputLabel id={`rating-label-${category.id}`}>Rating</InputLabel>
                                        <Select
                                    ...
                                    <Typography style={{ margin: '0 10px' }}>
                                        Calculated Rating: {ratings[category.id]?.calculatedRating || 0}
                                        <br />
