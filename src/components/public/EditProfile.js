...
export default function EditProfile() {
        ...
        if (jwtToken) {
          ...
          try {
            const response = await axios.get(
              `https://budget-planner-backend-c5122df5a273.herokuapp.com/api/v1/users/${fetchedUserId}`,
              {
                headers: {
  ...
  const handleUpdateProfile = async (event) => {
    ...
    try {
      await axios.put(
        `https://budget-planner-backend-c5122df5a273.herokuapp.com/api/v1/users/${userId}`,
        updatePayload,
        {
          headers: {
            Authorization: `Bearer ${jwtToken}`,
